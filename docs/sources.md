...
Every production preset includes two complete Kuphaldt electrical textbooks and all 20 acquired OpenStax mathematics/science PDFs. The original five-book seed is listed below for historical provenance; it is no longer the full default. The additional PDFs and their notices are documented in [education acquisition evidence](acquisition-education.md). Their explanations, figures and exercises support repair, agriculture and medical understanding; they do not replace task-specific safety or clinical guidance.
...
The five files total **1,422,085,263 bytes** (about 1.42 GB), which was part of the original 1.57 GB seed; the expanded 16 GB and 64 GB defaults now contain 9.70 GB and 40.27 GB respectively before generated search and navigation. Original PDF figures, labels, worked examples, tables and exercises are retained. OWL does not replace these books with text extracts, compress their images, or split them into missing-context snippets. Search extracts text separately while linking to the intact PDF. External activities, videos and publisher web features linked from the books are not downloaded.
...
Additional representative pages from 15 OpenStax titles, seven FAO guides and Pro Git were rendered and visually inspected after verifying their full source hashes. The [illustration audit](illustration-audit.md) records the exact physical pages and visible figures.

Both small presets enforce at least 22 required critical textbooks and 40 required critical illustrated works. All five defaults currently include 23 direct textbooks and 50 illustrated teaching works overall (22 and 49 required critical respectively). Explicit include/exclude customization may remove books; it never changes the ordinary-format requirement for assets marked critical. The builder prioritizes required critical teaching material before large archives.
