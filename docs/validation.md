...
## Embedded simulation scripts (2026-09-18)

The real SSD build exposed an extraction failure at asset 424 of 436: a pinned
PhET circuit simulation contains a legitimate embedded script of roughly 3 MB.
The HTML parser retained that script until its closing tag, hitting the existing
1 MiB malformed-token limit even though scripts are excluded from search text.
The extractor now discards ignored script/style bodies in bounded chunks while
preserving split closing tags. It still rejects oversized malformed HTML tokens
and never executes the embedded code.

The full local suite passed **381 tests**, including seven new regressions for
large scripts/styles, closing-tag boundaries, native parser semantics, hidden
text, entities and retained malformed-token limits. Read-only extraction of both
pinned PhET files succeeded. This establishes extraction of their static text,
not text generated by running the simulations.

## Two-object fresh-drive layout (2026-09-19)

The isolated layout change passed the complete **402-test** suite.

A fresh build publishes only `START_HERE.html` and `LIBRARY/` at its outer root.
Catalog destinations remain relative to `LIBRARY`; the global checksum manifest
inside that folder uses paths relative to the outer root and includes the start
page. Private checkpoints, partial downloads and copy state also stay inside
`LIBRARY/.owl`. Existing unrelated files are preserved.

A strict-atlas demo and its copied library each passed standalone isolated
verification with **89 OK, 0 missing, 0 failed, 0 unknown**. Post-download atlas
regeneration passed. Fresh offline Chrome 153 passed **18 search/navigation
checks** across desktop and 390px layouts, including JavaScript-disabled static
navigation, with no network requests, file picker, failed local requests or
browser errors. Both the outer landing page and `LIBRARY/SEARCH.html` returned
working content links. Renaming a separate demo's outer directory and rebuilding
reused its source files and complete index without downloads or re-extraction.

The previous physical SSD build was deliberately deleted by the user. These
results describe fresh local fixtures and do not claim a completed physical SSD.
