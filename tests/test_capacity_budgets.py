...
from owl.search import SearchError, build_search, checkpoint_usage
...
class CapacityBudgetTests(unittest.TestCase):
    ...
    def test_preflight_assigns_raw_to_target_and_database_to_work(self):
        ...
        with patch('owl.build.check_space_groups', return_value=[]) as check:
        ...
        allocations = dict(check.call_args.args[0])
        self.assertEqual(allocations[work], 2_250_000)
        self.assertEqual(allocations[target], self.asset['size_bytes'] + 1_000_000 + 750_000 + 100_000 + 16 * 1024 * 1024)
        self.assertFalse(target.exists())
